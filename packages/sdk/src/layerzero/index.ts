...
export function buildAdapterParams(
  ...
  nativeDropAmount: bigint = BigInt(0),
  nativeDropAddress: string = "0x0000000000000000000000000000000000000000"
): string {
 ...
 */
export function validateBridgeParams(params: OFTBridgeParams): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  if (!validChainIds.includes(params.srcChainId as any)) {
    errors.push(`Unsupported source chain ID: ${params.srcChainId}`);
  }
  if (!validChainIds.includes(params.dstChainId as any)) {
 ...
 */
export function formatTransferSummary(params: OFTBridgeParams): string {
